function [] = NPL(yzad)
    ...
    y0(1) = snout([u(k-tau) u(k-tau-1) y(k) y(k-1)]);
    y0(2) = snout([u(k-tau+1) u(k-tau) y0(1) y(k)]);
    y0(3) = snout([u(k-tau+2) u(k-tau+1) y0(2) y0(1)]);
    for i = 4:tau
        y0(i) = snout([u(k-tau+i-1) u(k-tau+i-2) y0(i-1) y0(i-2)]);
    end
    y0(tau+1) = snout([u(k-1) u(k-2) y0(tau) y0(tau-1)]);
    for i = tau+2:N
        y0(i) = snout([u(k-1) u(k-1) y0(i-1) y0(i-2)]);
    end
    for i = 1:D
        ys = circshift(ys,-1);
        us = circshift(us,-1);
